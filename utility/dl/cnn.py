...
def getVgg16( in_shape, layers ):
    ...
    """
    construct pretrained vgg16 model with customised fc and output layers
    """

    # load prexisting model
...
def getResNet50( in_shape, layers ):
    ...
    """
    construct pretrained resnet50 model with customised dense / dropout layers
    """

    # load prexisting model
    model = ResNet50(include_top=False, input_shape=in_shape)

    # freeze base model - with fix for batch normalisation weirdness
    # see: http://digital-thinking.de/keras-transfer-learning-for-image-classification-with-effificientnet/
    for layer in model.layers:
...
def getInceptionV3( in_shape, layers ):

    """
    construct pretrained inceptionv3 model with customised dense / dropout layers
    """
